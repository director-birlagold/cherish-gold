...
class Spreadsheet_Excel_Reader {
	...
	// DUMP AN  ENTIRE XLS DATA
	// =========================================
	function dumpdata($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {		
   		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				if($row=='1'){
 				if(trim($this->val($row,$col,$sheet))!=''):
				$data[]= $this->val($row,$col,$sheet);				
				endif;
				}else{
					//if(trim($this->val($row,$col,$sheet))!=''):
					$key=$data[$col-1];					
					$edata[$key]=$this->val($row,$col,$sheet);
					//endif;
				}
   			}
			if($row!='1'){
