<div id="content" class="clearfix"> 
    <div class="container">
        <div class="mainheading">   
            ...
            <div class="titletag"><h1><?php echo __('Customer Master'); ?></h1></div>
        </div>
       ...
       <th width="30" align="center">View</th>
		...
		?>
        <tr>
        <!-- td align="center"><input type="checkbox" name="action[]" value="<?php echo h($customer['CustomerBGP']['customer_id']); ?>"  class="validate[minCheckbox[1]] checkbox" rel="action" /></td> -->
        <td align="center"><?php echo h($i); ?></td>
        <td align="left"><?php  echo $customer['CustomerBGP']['application_no'];?></td>
         <td align="left"><?php  echo $customer['CustomerBGP']['partner_code'];?></td>
		 <td align="left"><?php  echo $customer['CustomerBGP']['applicant_name'];?></td>
		 <td align="left"><?php  echo $customer['CustomerBGP']['contact_mobile'];?></td>
		 <td align="left"><?php  echo $customer['CustomerBGP']['contact_email'];?></td>
		 <td align="left"><?php  echo $customer['CustomerBGP']['initial_pay_by'];?></td>
		 
		 <td align="left"><?php  
		 if($customer['CustomerBGP']['initial_pay_by'] != "Cheque") 
		 { 
			if(isset($customer['CustomerBGP']['f_code'])) 
			{ 
				if($customer['CustomerBGP']['f_code'] == "Ok") 
					{ 
						echo "Success"; 
					}
				else 
		...
		{ 
		 ...
		 ?></td>
		 <td align="left"><?php  if($customer['CustomerBGP']['approve_status'] == "Rejected") { ?> <span title="<?php echo $customer['CustomerBGP']['reason']; ?>"> <?php echo $customer['CustomerBGP']['approve_status']; ?> </span><?php } else { echo $customer['CustomerBGP']['approve_status']; }?></td>
		 
        <td align="center">
			<?php echo $this->Html->image('icons/edit.png',array('url'=>array('action'=>'view', $customer['CustomerBGP']['customer_id']),'border'=>0,'alt'=>__('Edit')) );?>
			<a href="javascript:void(0);" onclick="downloadform('<?php echo $customer['CustomerBGP']['customer_id']; ?>');" title="Download Form">Form</a>&nbsp;&nbsp;
			<a href="javascript:void(0);" onclick="downloadAC('<?php echo $customer['CustomerBGP']['customer_id']; ?>');" title="Download Form">ACK</a>
			</td>
      <!--  <td align="center"><?php echo $this->Html->image('icons/cross.png',array('url'=>array('action'=>'delete',$customer['CustomerBGP']['customer_id']),'border'=>0,'class'=>'confirdel','alt'=>__('Delete')) );?></td> -->
        </tr>
        <?php $i++; endforeach;
        }
        ?>
