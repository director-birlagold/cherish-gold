...
class WebpagesController extends AppController {
    ...
    public function index() 
	{
		...
		if (isset($this->request->data['giveaway-email'])) {
            if ($this->request->is('post')) {
				...
				$giveawaynewsletter = (isset($this->request->data['giveaway-newsletter'])) ? "Yes" : "No";
				$message = "Hello<br/><br/><br/>You have a request with below details <br/><br/>Email :- ".$this->request->data['giveaway-email']."<br/>Sign Up for offer :- ".$giveawaynewsletter."<br/><br/><br/>Thank You!";
				$this->mailsend(SITE_NAME, $adminmailid['Adminuser']['email'], $adminmailid['Adminuser']['email'], "Give Away Newsletter", $message);
				setcookie("shagunnpop", "1", time() + (86400 * 365), "/");
			}
		}
    }
    ...
    public function product() {
        ...
        $field = array('ROUND(ROUND((metal_purity/24)*(SELECT price FROM sha_price WHERE metal_id=1 AND metalcolor_id=Product.metal_color_id AND metal_fineness=Product.metal_fineness))*metal_weight) AS metalprice',
		 ...
		 IF(making_charge_calc = \'PER\', ROUND(ROUND(ROUND((metal_purity/24)*(SELECT price FROM sha_price WHERE metal_id=1 AND metalcolor_id=Product.metal_color_id AND metal_fineness=Product.metal_fineness))*metal_weight)*making_charge/100), making_charge)+
...
//            'Product.*');
        ...
        if (!empty($this->params['pass']['0'])) {
			...
			$assistance_type = $this->ShoppingAssistance->find('first', array('conditions' => array('LOWER(title)' => str_replace('_', ' ', $this->params['pass']['0']))));
            if (!empty($assistance_type)) {
                $conditions[] = 'FIND_IN_SET(' . $assistance_type['ShoppingAssistance']['shopping_assistance_id'] . ',Product.shopping_assistance_type)';
			}
        }
		...
		$url =  $_SERVER['REQUEST_URI'];
			...
			if(strpos($url[count($url)-1],'type')!==false)
			{
				if($url[count($url)-1]=='type-diamond')
				{
					$conditions[] = 'FIND_IN_SET(2,Product.product_type)';
				}
				else if($url[count($url)-1]=='type-gold')
				{
        ...
        if (!empty($_GET)) {
            ...
            if (!empty($_GET['search'])) {
                ...
                if (!empty($category)) {
					...
					//print_r($category['Category']['category']);
					//$order = $category['Category']['category'].'DESC';
...
//                        }
            ...
            if (!empty($_GET['goldpurity'])) {
				...
				if($_GET['goldpurity'] == 'all'){
					...
					$joins[] = array(
						...
						'type' => 'inner',
						'foreignKey' => false,
						'conditions' => array('`Productmetal.product_id`=`Product.product_id`', 'Productmetal.type' => 'Purity')
					);
            ...
            if (!empty($_GET['gemstone'])) {
				...
				if($_GET['gemstone'] == 'all'){
					$joins[] = array(
						'table' => 'productgemstone',
						'alias' => 'Productgemstone',
						'type' => 'inner',
						'foreignKey' => false,
						'conditions' => array('`Productgemstone.product_id`=`Product.product_id`')
					);
				}else{
					$joins[] = array(
						'table' => 'productgemstone',
            ...
            if (!empty($_GET['shape'])) {
                if (!empty($_GET['gemstone']) && $_GET['gemstone'] !='all' ) {
				...
				if($_GET['shape'] == 'all'){
					...
					$joins[] = array(
						...
						'type' => 'inner',
						'foreignKey' => false,
						'conditions' => array('`Productshape.product_id`=`Product.product_id`', $gdetails)
					);
					$joins[] = array(
						...
						'type' => 'inner',
						'foreignKey' => false,
						'conditions' => array('`Productdiamond.product_id`=`Product.product_id`')
					);
					...
					$joins[] = array(
						...
						'type' => 'inner',
						'foreignKey' => false,
						'conditions' => array('`Productshape.product_id`=`Product.product_id`', 'Productshape.shape' => $_GET['shape'], $gdetails)
					);
					$joins[] = array(
						'table' => 'productdiamond',
						'alias' => 'Productdiamond',
...
//		 IF(Product.gemstone=\'Yes\',ROUND((SELECT SUM(Productgemstone.stone_weight*(SELECT Price.price FROM sha_price AS Price WHERE Price.gemstone_id=(SELECT Gemstone.gemstone_id FROM sha_gemstone AS Gemstone WHERE Gemstone.stone=Productgemstone.gemstone) AND Price.gemstoneshape=(SELECT Shape.shape_id FROM sha_shape AS Shape WHERE Shape.shape = Productgemstone.shape ))) FROM sha_productgemstone AS Productgemstone WHERE product_id=Product.product_id)),0))*vat_cst/100)) ' . $cod;
 ...
 public function enquries() {
    ...
    public function admin_homeenquries_export($cdate,$edate) {
		...
		if($cdate == 0 && $edate == 0){		
			...
			$results = $this->Enquries->find('all', array('conditions' => array('status !=' => 'Trash','created_date <='=>$edate)));
		}else if($edate==''){	
			$results = $this->Enquries->find('all', array('conditions' => array('status !=' => 'Trash','created_date >='=>$cdate)));
		}else{	
			$results = $this->Enquries->find('all', array('conditions' => array('status !=' => 'Trash','created_date >='=>$cdate,'created_date <='=>$edate)));	
		}
    ...
    public function admin_question() {
        ...
        if ($this->request->query('search') != '') {
            $search = array();
			if (($this->request->query('cdate') != '') && ($this->request->query('edate'))) {
                $search = array_merge($search, array('Question.created_date BETWEEN \'' . $this->request->query('cdate') . '\' AND \'' . $this->request->query('edate') . '\''));
            } elseif ($this->request->query('edate') != '') {
                $search = array_merge($search, array('Question.created_date >=' => $this->request->query('edate')));
            } elseif ($this->request->query('cdate')) {
                $search = array_merge($search, array('Question.created_date <=' => $this->request->query('cdate')));
            }
    ...
    public function whislist() {
        ...
        if ($this->Session->check('loginid')) {
            ...
            $whish = $this->Whislist->find("list", array('conditions' => array('Whislist.user_id' => $this->Session->read('User.user_id'), 'Whislist.status' => 'Active', 'Whislist.product_id' => $this->params['pass']['1']))); 
            ...
            if (empty($whish)) {
                ...
                $this->Session->setFlash("<div class='success msg'>" . __('Product has been successfully added in wish list.') . "</div>", '');
                //$this->redirect(array('action' => 'product', $this->params['pass']['0']));
                $wishlist_count = $this->Session->read('User.wishlist_count') + 1;
                $this->Session->write('User.wishlist_count',$wishlist_count);
                $this->redirect($this->referer());
            } else {
                $this->Session->setFlash("<div class='error msg'>" . __('This item already exists in your wish list.') . "</div>", '');
                //$this->redirect(array('action' => 'product', $this->params['pass']['0']));
                $this->redirect($this->referer());
            }
        } else {
...
//            'Product.*');
        ...
        if (!empty($_GET)) {
			...
			if(strpos($_GET['product_type'],'type')!==false)
			{
				if($_GET['product_type']=='type-diamond')
				{
					$conditions[] = 'FIND_IN_SET(2,Product.product_type)';
				}
				else if($_GET['product_type']=='type-gold')
				{
					$conditions[] = 'FIND_IN_SET(1,Product.product_type)';
				}
				else
...
//        $Email->send('My message');
	...
	// Added By Rohan on 29-01-2016
	
	public function opencart_api_product_details()
	{
		$this->layout = '';
...
//            $gold_price = round(round($goldprice['Price']['price'] * ($material[0] / 24)) * $tot_weight);
        ...
        $resp['json'] = $jsonarray;
		$resp['goldprice'] = $goldprice;
		$resp['product'] = $product;
		$resp['total_weight'] = $total_weight;
		$resp['metals'] = $metals;
		$resp['diamond'] = $diamond;
		$resp['purity'] = $purity;
		$resp['diamonddiv'] = $diamonddiv;
		$resp['Productmetal'] = $Productmetal;
		$resp['category'] = $category;
		$resp['formated_total'] = indian_number_format($total);
		$resp['total'] = $total;
		$resp['cart'] = $cart;
	...
	public function opencart_api_product_color_size_details()
	{
		$productid = $_REQUEST['product_id'];
		$product = ClassRegistry::init('Product')->find('first', array('conditions' => array('product_id' => $productid, 'status' => 'Active')));
		...
		$resp = json_encode(array('product'=>$product,'metals'=>$metals,'category'=>$category,'size'=>$size));
