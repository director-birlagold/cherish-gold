 ...
 */
	...
	public $uses=array('Order','User','Shipping','Shoppingcart','Discount','Paymentdetails','Discounthistory','Product','Productimage','Category','Productdiamond','Productgemstone','Vendorcontact','Products');
	   ...
	   public function order($id=''){
		   ...
		   if($id!='')
		   {
			...
			{				
				...
				$already_used_or_not=$this->Discounthistory->find('first',array('conditions'=>array('coupon_id'=>$discount_id,'user_id'=>$user_id))); 
		...
		if($this->Session->read('cart_process')!=''){		
		 ...
		 if($this->Session->read('User')!=''){
		   ...
		   if($this->Session->read('cart_process')!='')
			  ...
			  if($id!='' && $message!='')
		   {
		   $this->Session->setFlash("<div class='success msg'>" .$message. "</div>");
		   }
	   }
