...
class SubcategoriesController extends AppController {
	...
	public function admin_sub_category_export($term,$cterm) {
		$this->layout = '';
        $this->render(false); 
		ini_set('max_execution_time', 600); 
		$filename = "sub_category_details.csv";
		...
		$conditions['status !='] = 'Trash'; 
		if ($term != 0) {
			$conditions['subcategory LIKE'] = '%' . $term . '%'; 
		}		
		if ($cterm != 0) {    
			$conditions['category_id'] = $cterm;  
		}	
		$results = $this->Subcategory->find('all', array('conditions' => $conditions));
        $header_row = array("S.No", "Sub Category","Category", "Code", "Status"); 
		fputcsv($csv_file, $header_row, ',', '"');  
		$i = 1;  
		foreach ($results as $results) {		
			$category=$this->Category->find('first',array('conditions'=>array('category_id'=>$results['Subcategory']['category_id'])));
			$row = array($i,$results['Subcategory']['subcategory'],$category['Category']['category'],$results['Subcategory']['subcategory_code'],$results['Subcategory']['status']);
            $i++;  
        ...
        if ($this->request->is('post')) {
            ...
            if (empty($check)) {
                $this->request->data['Subcategory']['link'] = $page_link;
								$this->request->data['Subcategory']['subcategory'] =trim($this->request->data['Subcategory']['subcategory']);
