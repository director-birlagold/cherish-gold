    ...
    public function index() {
        ...
        $url = $this->payu_url($env);
            ...
            if ($orderamount['Order']['cod_status'] == 'COD') {
				...
				$amount=$orderamount['Order']['netpayamt'];
            }
		
		
        $order = $this->Order->find('first', array('conditions' => array('order_id' => $this->Session->read('Order'))));
        $user = $this->User->find('first', array('conditions' => array('user_id' => $this->Session->read('loginid'))));		
       ...
       /* if ($_SERVER['HTTP_HOST'] == 'localhost' || $_SERVER['HTTP_HOST'] == '192.168.1.20') {
        ...
        $params = array('url' => $url . '_payment', 'key' => $key, 'txnid' => $in . $order['Order']['invoice'], 'amount' => $amount, 'firstname' => $user['User']['first_name'], 'lastname' => $user['User']['last_name'], 'email' => $user['User']['email'], 'phone' => $user['User']['phone_no'], 'productinfo' => 'Shagunn Product', 'surl' => BASE_URL . 'orders/payment_success', 'furl' => BASE_URL . 'orders/payment_failure', 'salt' => $salt);
